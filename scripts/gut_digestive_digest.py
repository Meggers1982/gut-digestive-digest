    ...
    try:
        headers = {
            "User-Agent": (
                ...
                "+https://github.com/Meggers1982/gut-digestive-digest)"
            ),
...
{{
  ...
  "groundbreaking": "one or more of: Counterintuitive finding / Overturns prior research / First-in-class human study / Major dietary or microbiome implication",
  "media_coverage": "{media_note}",
  "summary": "2-4 sentences: what researchers did, who or what was studied (N=X, human vs. animal vs. in vitro), key finding in plain language — verified against abstract",
  "why_it_matters": "1-2 sentences of real-world significance for people interested in gut health, diet, or digestive disease. Do NOT imply clinical action.",
  "caveats": "comma-separated flags: small sample (N<100), observational design, single-center, self-reported dietary intake, short follow-up, industry funding [name], preprint, secondary analysis, animal or cell study, microbiome data only (no clinical outcomes) — or 'None identified'",
  "fact_check_note": "Describe any corrections made vs what the abstract actually says, or empty string",
  "excluded": false,
  "relevance_score": 7,
  "relevance_score_reason": "One sentence explaining the score — what makes it strong or weak for gut and digestive health journalism",
  "pitch_angles": [
    {{
      ...
      "pitch_angle": "2-3 sentence commissioning pitch: what happened, why it matters for gut health or diet, broader lifestyle or scientific significance",
...
- For microbiome studies: note whether findings are based on 16S sequencing, shotgun metagenomics, or culture — and whether clinical outcomes were measured or only microbiome composition changed
- For nutrition: specify whether the study tested whole dietary patterns, individual foods, or supplements, and at what dose and duration
- For microbiology: distinguish between human pathogens, commensal bacteria, and environmental microbes — context matters enormously
- For gastroenterology: specify the condition (IBD, IBS, GERD, colorectal cancer, celiac, etc.) — do not use "gut issues" generically
- Microbiome findings are often preliminary — flag when human causal data is absent and only associations or animal/in vitro data exist
...
relevance_score rubric (1–10): start at 5, then adjust:
  +2 counterintuitive or overturns widely held belief about diet, the microbiome, or digestion
  +2 human subjects with clinical or dietary outcomes (N≥100)
  +1 clear everyday relevance (fermented foods, fiber, probiotics, ultra-processed food, intermittent fasting)
  +1 clean design (RCT, longitudinal cohort, well-controlled intervention)
  +1 links gut health to a systemic outcome (brain, immune function, metabolic health, mood)
  −1 per major caveat
  −1 microbiome composition change only, no clinical outcome data
...
{studies_block}"""
    ...
    try:
        ...
        return [{"pmid": s["pmid"], "headline": s["title"], "journal": s["journal"],
                 ...
                 "groundbreaking": "Relevant gut health finding", "media_coverage": media_note,
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    subject = f"Gut & Digestive Health Research Digest — {run_date} | {study_count} {'Study' if study_count == 1 else 'Studies'}"
    html = f"""<!DOCTYPE html>
<html>
<body style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:24px;color:#222;">
<h2 style="color:#1a3a2e;">Gut &amp; Digestive Health Research Digest</h2>
...
<p style="font-size:0.85em;color:#888;margin-top:2em;">
  Gut &amp; Digestive Health Research Digest · PubMed + Claude + SERPAPI
</p>
</body>
</html>"""
    try:
...
def main():
    ...
    print("=" * 60)
    print("Gut & Digestive Health Research Digest")
